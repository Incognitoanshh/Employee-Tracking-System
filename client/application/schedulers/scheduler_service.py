    ...
    def _sync_tick(self):
        ...
        if self._sync_counter >= self._sync_interval:
            try:
                ...
                print(f"[SYNC BACKOFF] failure #{self._sync_failures}, next retry in {self._sync_interval}s")
            self._sync_counter = 0
