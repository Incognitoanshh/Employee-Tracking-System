...
class ScreenshotManager:
    ...
    def generate_random_schedule(
        ...
        while len(timestamps) < count and attempts < count * 20:
            attempts += 1
            offset = random.uniform(buffer, buffer + available)
            candidate = shift_start + timedelta(seconds=offset)

            too_close = any(
                abs((candidate - t).total_seconds()) < min_gap for t in timestamps
            )
            if not too_close:
