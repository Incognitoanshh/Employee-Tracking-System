...
class _AttendanceFetchWorker(QThread):
    ...
    def run(self):
        try:
            # BUG FIX: pehle yahan koi employee_id filter nahi tha, is liye
            # employee ki Attendance History window /attendance/all se SAARE
            # employees ke shift records khinch leti thi — apne records ke
            # saath baaki sab ke bhi rows table mein dikhte the, aur
            # Today/Week/Month cards bhi sab employees ke total time jod ke
            # dikhate the (galat, inflated numbers). Ab sirf apna
            # employee_id query param bhejte hain — ShiftManager
            # (_has_open_server_session) aur admin ke _AttendanceTab dono
            # already isi pattern se employee_id filter karte hain.
            response = requests.get(
                f"{API_BASE_URL}/attendance/all",
                headers={"Authorization": f"Bearer {SessionManager.auth_token}"},
                params={"employee_id": SessionManager.employee_id},
                timeout=10,
            )
